...
SunriseSunset.prototype = {
	...
	getDOY : function() {
		var month = this.utcMonth, year = this.utcFullYear, day = this.utcDay;
	...
	sinDec : function() {
		var L = this.trueLongitude(), sinDec = 0.39782 * this.sin(L);
	...
	localMeanTime : function() {
		var cosH = (this.cos(this.zenith) - (this.sinDec() * this
				.sin(this.latitude)))
	...
	// utcCurrentHours is the time that you would like to test for daylight, in
	// hours, at UTC
	// For example, to test if it's daylight in Tokyo (GMT+9) at 10:30am, pass
	// in
	// utcCurrentHours=1.5, which corresponds to 1:30am UTC.
	isDaylight : function(utcCurrentHours) {
		var sunriseHours = this.sunriseUtcHours(), sunsetHours = this
				.sunsetUtcHours();
...
function SunriseSunsetTest() {
	...
	for ( var city_name in testcases) {
		...
		for ( var i = 0; i < city.tests.length; i++) {
			var t = city.tests[i];
			var ss = new SunriseSunset(t.year, t.month, t.day, city.lat,
					city.lon);
			...
			/* jsl:ignore */
			print(city_name, t.year, t.month, t.day, t.utcHours, "passed:",
					passed);
			if (!passed) {
