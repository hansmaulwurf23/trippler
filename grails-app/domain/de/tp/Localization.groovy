...
class Localization implements Serializable{
    ...
    static mapping = {
		table name: "app_localization"
		id generator:'sequence', params:[sequence:'localization_seq', initialValue:1]
        columns {
    ...
    static constraints = {
        ...
        locale(blank: false, unique: 'code', matches: "\\*|([a-z][a-z]([A-Z][A-Z])?)")
	...
	static namedQueries = {
		locales {
			projections { distinct( 'locale' ) }
		}
		codes {
			projections { distinct( 'code' ) }
		}
	}
	
	static addMissing(String code) {
    	if (!missings.contains(code)) {
			synchronized (missings) {
				missings.add(code)
			}
    	}
	}
	static removeMissing(String code) {
		if (missings.contains(code)) {
			synchronized (missings) {
				missings.remove(code)
			}
		}
	}
	static resetMissings() {
		synchronized (missings) {
			missings = []
		}
	}
    ...
    static String decodeMessage(String code, Locale locale) {
        ...
        if (!msg) {
            def lst = Localization.findAll(
                    "from Localization as x where x.code = ? and x.locale in ('*', ?, ?) order by x.relevance desc",
